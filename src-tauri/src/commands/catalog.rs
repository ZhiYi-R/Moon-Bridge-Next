...
pub struct DetectResult {
    /// 数据来源：QQliveQQ（实时探测）/ QQcatalogQQ（models.dev 目录回退）。
    pub source: String,
...
/// 按协议构造模型列表探测 URL；QQNoneQQ 表示该协议不支持实时探测。
///
/// OpenAI 兼容端点的 baseUrl 通常自带 QQ/v1QQ 后缀，模型列表在 QQ{base}/modelsQQ；
/// Anthropic 的 base 不带 QQ/v1QQ，列表在 QQ{base}/v1/modelsQQ。
...
/// QQ{ "data": [{ "id": ... }] }QQ 形态，Anthropic 另有 QQdisplay_nameQQ。
...
pub async fn provider_detect_models(
    ...
    let base_urls: Vec<&str> = provider.endpoints.iter().map(|e| e.base_url.as_str()).collect();
    let models_dev_id = crate::presets::find_preset(&provider.key, &base_urls)
    ...
    let ids = match live {
        ...
        Ok(_) => return catalog_fallback("上游返回空列表".to_string(), catalog, catalog_err, &existing),
    ...
    let warning = if models_dev_id.is_none() {
        ...
        Some(format!("models.dev 目录拉取失败（{e}），仅返回实时探测的裸列表"))
...
mod tests {
    ...
    fn probe_url_per_protocol() {
        ...
        assert!(probe_url("google-genai", "https://x").is_none(), "未覆盖协议不支持探测");
        assert!(probe_url("openai-chat", "  ").is_none(), "空 base 不构造 URL");
    }
    ...
    fn parse_live_ids_openai_and_anthropic() {
        ...
        assert_eq!(
            ...
            vec![("claude-sonnet-5".to_string(), Some("Claude Sonnet 5".to_string()))]
        );
