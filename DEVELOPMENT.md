...
# 无桌面系统库的环境（如 CI/服务器）：排除 Tauri 壳，其余照常全绿
...
# 无头服务端（web 模式的后端，同端口提供网关 + /api/* + ./ui/dist）
...
- **前端运行时三态**：所有 command 调用收敛在 `ui/src/lib/api.ts` 的 `call()` 分发层——Tauri 壳内走真实 IPC；纯浏览器默认走 `ui/src/lib/web.ts` 的 REST 客户端（连真实服务端，token 存 localStorage，401 自动弹登录卡）；仅 `VITE_USE_MOCK=1` 时切到 `ui/src/lib/mock.ts` 的内存实现（数据刷新即重置，写操作在会话内生效）。
...
根目录 `Dockerfile` 多阶段构建（node 构建 ui → rust 编译 server → debian-slim 运行时，uid 10001，`EXPOSE 38440`，`VOLUME /data`）：
...
- `./data` 挂进容器前必须 `chown -R 10001:10001`（容器以 uid 10001 运行，否则无法写库）。
- admin token 由 `MOONBRIDGE_ADMIN_TOKEN`（或 `--admin-token`）注入，只驻留内存、不落盘；它同时是 LLM 入口与 `/api/*` 的 Bearer token。
- 「重启网关」按钮 = 进程退出（exit 0），靠 `restart: unless-stopped` 复活，务必保留该策略。
- soul 部署样例（compose + .env.example + 1panel-network）见 `deploy/soul/`。

## 测试基线

当前 `cargo test --workspace` 全绿（345 项，覆盖 core / protocol / plugin / store / gateway e2e+balance / server / app；无桌面系统库时 `--exclude moonbridge-app` 为 331 项），`cargo check --workspace --all-targets` 与 `cargo clippy --workspace --all-targets` 零告警，`pnpm --dir ui type-check` 无错。提交前请保持这个状态。
